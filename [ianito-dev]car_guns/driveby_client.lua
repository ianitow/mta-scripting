...
isAllowed = nil
function isPlayerAllowedToUse(isAllowed_c)
	isAllowed = isAllowed_c
	return isAllowed_c
end

addEvent( "isPlayerAllowedToUse", true )
addEventHandler( "isPlayerAllowedToUse", localPlayer, isPlayerAllowedToUse )

--This function handles the driveby toggling key.
function toggleDriveby()
	--Check in ACL
	triggerServerEvent ( "checkInACL", resourceRoot, getPlayerName(localPlayer) )
	if not (isAllowed) then outputChatBox(MESSAGE_NOT_ALLOWED,255,255,255,true) return end 
	...
	if settings.enabled and equipedWeapon == 0 then
		...
		if prevw and nextw then
			...
			helpText:text( "Pressione '"..prevw.."' ou '"..nextw.."' para trocar de arma" )
			fadeInHelp()
