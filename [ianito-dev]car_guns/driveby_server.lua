...
function checkInACL ( playerName )
	if(isAllowed(getPlayerFromName(playerName))) then
		triggerClientEvent ( getPlayerFromName(playerName), "isPlayerAllowedToUse", getPlayerFromName(playerName), true )
	else
		triggerClientEvent ( getPlayerFromName(playerName), "isPlayerAllowedToUse", getPlayerFromName(playerName), false )
	end
end
addEvent( "checkInACL", true )
addEventHandler( "checkInACL", resourceRoot, checkInACL) 


function isAllowed(player)
	local accName = getAccountName ( getPlayerAccount ( player ) )
	for i,k in pairs(ACL_ALLOWED) do
