...
function toggleDriveby()
	...
	if equipedWeapon == 0 then
		...
		if (getElementData(localPlayer,"[ianito-dev]isPlayerProtected")) then return end
		...
		if prevw and nextw then
			...
			fadeInHelp()
			setElementData(localPlayer,"isDriveShootingEnabled",true)
		...
		setElementData(localPlayer,"isDriveShootingEnabled",false)
		fadeOutHelp()
